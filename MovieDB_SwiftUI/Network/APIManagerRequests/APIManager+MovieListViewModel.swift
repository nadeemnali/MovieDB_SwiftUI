//
//  APIManager+MovieListViewModel.swift
//  MovieDB_SwiftUI
//
...
extension MovieListViewModel {
    ...
    func getMovieListData(completionHandler: @escaping (NetworkResult<MovieListRequest.ResponseType>) -> Void) {
        if AppSettings.sandbox {
            let jsonString = getMovieListOfflineJsonString()
            do {
                let decoder = JSONDecoder()
                decoder.keyDecodingStrategy = .useDefaultKeys
                let jsonData = jsonString.data(using: .utf8)!
                let response = try decoder.decode(MovieListRequest.ResponseType.self, from: jsonData)
